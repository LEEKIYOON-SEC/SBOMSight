...
| 페이지 | 내용 |
|---|---|
| `/` | 자산 대시보드 — 서버별 마지막 스캔·경과일·미분류 스캔 배정 |
| `/asset.html` | 자산 상세 — 스캔 이력 · 두 스캔 비교(신규·해소·유지) · 자산 정보 |
| `/scan.html` | 업로드 · 진행 스테퍼 · 결과 표(체크박스 · 우선순위/패키지 유형/상태 필터) · CVE 상세 · 이그레스 미리보기 |
| `/report.html` | 보고서 뷰어 · 목차 · 인쇄(PDF) · Markdown/HTML 내려받기 |
| `/login.html` | 로그인 · 최초 관리자 계정 생성 |
| `/settings.html` | 계정 관리 · 접근 IP · 내 비밀번호 (관리자) |

### 자산이 관리 단위다

이 도구의 관리 단위는 스캔이 아니라 **서버 한 대**다. `web-01` 의 SBOM 을 매달
올리고, 지난달 대비 무엇이 새로 생기고 무엇이 사라졌는지를 보는 것이 실제 운영이다.
"최근 스캔 목록"으로는 그 질문에 답할 수 없다.

```bash
python -m core.cli asset add web-01 --group DMZ --os "Rocky 9.3"
python -m core.cli asset list                 # 자산별 마지막 스캔과 경과
python -m core.cli asset assign <스캔ID> web-01
python -m core.cli asset history web-01       # 최근 두 스캔 대조
```

이력 비교의 축은 `(cve, 설치 패키지명)` 이다. `Finding.key` 는 설치 버전을 포함하므로
패치하면 키가 바뀐다 — 그것으로 비교하면 부분 패치가 "해소 1건 + 신규 1건"으로
갈라져 화면이 거짓말을 한다. 결과는 **신규 · 해소 · 유지** 세 갈래로 낸다.
"187건에서 184건이 됐다"만으로는 같은 수라도 내용이 완전히 바뀐 경우를 구분할 수 없다.

담당자 필드는 두지 않는다 — 업무분장에 따라 바뀌는 것은 자산의 속성이 아니다.
자산을 삭제해도 그 자산의 스캔은 지워지지 않고 미분류로 남는다.

### 로그인과 접근 통제

스캔 결과는 "어느 서버에 어떤 취약점이 열려 있는가"다. 내부망이라는 이유로 아무나
열어 볼 수 있게 두지 않는다. 두 층이 순서대로 선다.

```
소켓 상대 IP 확인  →  세션 확인  →  권한 확인  →  화면
 (허용 대역 밖이면 로그인 화면도 안 보인다)
```

**최초 1회** — 계정이 하나도 없으면 서버 PC 자신(`127.0.0.1`)에서 접속했을 때만
관리자 생성 화면이 열린다. 네트워크의 누군가가 먼저 와서 관리자를 차지할 수 없다.
CLI 로도 만들 수 있다.

```bash
python -m core.cli user add <이름> --role admin   # 비밀번호는 프롬프트로 입력
python -m core.cli user list
```

**계정 관리**는 `설정 → 계정`에서 관리자가 한다 — 생성·삭제·비밀번호 초기화·권한 변경.
권한은 둘이다: `admin`(전체)과 `viewer`(읽기 전용 — 스캔·삭제·계정 관리 불가).
비밀번호는 `hashlib.scrypt` 로 저장하며 표준 라이브러리 외에는 아무것도 쓰지 않는다.

**세션은 놀고 있으면 끊긴다.** 마지막 요청으로부터 **10분**(`SBOMSIGHT_SESSION_IDLE_MINUTES`)
이 지나면 서버가 세션을 지우고, 화면은 로그인으로 돌아가며 왜 돌아왔는지 알린다.
쓰고 있는 동안에는 요청이 계속 나가므로 밀려나지 않는다 — 긴 표를 읽고만 있어서
요청이 없을 때는 화면이 조작(누르기·타이핑·스크롤)을 봐서 살아 있다고 알린다.
계속 쓰더라도 로그인으로부터 12시간(`SBOMSIGHT_SESSION_TTL_HOURS`)이 지나면 끊긴다.

**접근 IP**는 `설정 → 접근 IP`에서 CIDR 목록으로 관리한다(`SBOMSIGHT_ALLOWED_IPS`
는 최초 부트스트랩용). 판단은 **소켓 상대 주소로만** 하며 `X-Forwarded-For` 같은
헤더는 보지 않는다 — 누구든 채워 보낼 수 있어서, 그것을 믿으면 목록이 헤더 한 줄로
우회된다. 목록이 비면 제한 없음이고, 지금 접속 중인 주소가 빠진 목록은 저장되지
않는다(자기 잠금 방지).

기본 바인딩은 `127.0.0.1`이다. `SBOMSIGHT_HOST=0.0.0.0` 또는
`run-server.ps1 -Listen` 으로 내부망에 여는데, **계정이 하나도 없으면 열지 않는다.**
