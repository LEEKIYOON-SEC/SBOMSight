...
**목표 버전은 하나로 고르지 않는다.** 한 패키지에 걸린 취약점마다 수정 버전이
다를 수 있고(curl 한 벌에 여덟 가지), 그중 어느 것이 그 서버에 맞는지를 버전끼리
견주어 정하는 것은 도구가 할 판단이 아니다. 여럿이면 `수정 버전 N가지` 와 그
목록을 적는다 — 조치 화면 · 보고서 · CSV 가 같다. **목록은 글자 순이지 버전 순이
아니다**(`9.0.107` 이 `9.0.68` 보다 앞에 온다).
...
rpm -q <package>             # 기대값: 보고서의 목표 버전 이상 (여럿이면 그중 가장 높은 것 이상)
...
**먼저 양쪽 grype 버전을 맞춘다.** DB 에는 스키마 번호가 있고 grype 은 자기가 아는
스키마만 읽는다. 버전이 다르면 반입해도 안 쓰이거나 갱신을 다시 시도한다.

인터넷 되는 장비에서:

```bash
grype version                    # 버전을 적어 둔다
grype db update
grype db status
```
...
**설정 → 도구 상태** 에서도 grype 버전이 잡히는지 본다.
...
검사마다 grype 버전과 취약점 DB 기준일을 함께 저장한다. 몇 달 뒤 같은 SBOM 의
결과가 달라진 이유를 설명할 수 있어야 하기 때문이다. 화면과 보고서 머리에
그대로 찍힌다.

구역 보고서는 한 걸음 더 간다 — **기간 안의 검사가 서로 다른 grype 버전이나 다른
DB 기준일로 돌았으면 그 사실을 보고서에 적는다.** 새 버전이 규칙을 더 가지면 같은
...
| DB 설치 파일 (MariaDB 또는 MySQL 8) | 약 100~450MB | 한 번 |
| SBOMSight 소스 (또는 빌드된 jar) | 약 10MB / 약 60MB | 갱신할 때마다 |
| Maven 의존성 | 약 80MB | 한 번 (`~\.m2` 를 통째로 반입) |
| grype 실행 파일 | 약 30MB | 업그레이드할 때 |
| syft 실행 파일 (대상 서버용) | 약 30MB | 업그레이드할 때 |
...
패키지 목록(`패키지` 화면)은 **검사할 때 SBOM 에서 담긴다.** 업그레이드가 이미
쌓인 SBOM 을 되읽지는 않으므로, 업그레이드 직후에는 이 화면이 비어 있다. 그 상태를
"패키지가 없습니다" 라고 말하지 않고 **"아직 담긴 패키지가 없습니다"** 라고
띄운다 — 없는 것이 아니라 안 본 것이다.

자산마다 검사 이력의 **`다시 검사`** 를 한 번 누르면 채워진다.

- **SBOM 을 다시 업로드할 필요가 없다.** 보관해 둔 원본을 그대로 다시 읽는다.
- 반입한 grype DB 로 다시 대조하므로 **취약점도 같이 최신이 된다.**
- 자산마다 **최신 검사 것만** 남는다. 여러 번 눌러도 행이 쌓이지 않는다.

> 자산이 많으면 한 번에 다 누르지 말고 구역 단위로 나눠 돈다. grype 한 번이
> 몇 분이고, 동시에 여러 개를 돌리면 그만큼 오래 걸린다.
...
운영 절차(서비스 · 리듬 · 백업 · 업그레이드)는 [`docs/operations.md`](operations.md) 다.
