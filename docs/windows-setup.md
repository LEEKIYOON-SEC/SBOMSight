...
가장 중요한 신호는 **버전 갈림** 딱지다. 같은 패키지에 취약한 버전과 안전한
버전이 섞여 있다는 뜻이고, 곧 "이미 올린 자산이 있는데 안 올린 자산이 남았다"
는 뜻이다. `버전이 갈린 것만` 거르개로 그것만 볼 수 있다.

> **판올림 직후에는 이 화면이 비어 있다.** 패키지 목록은 검사할 때 SBOM 에서
> 담기고, 이미 쌓인 SBOM 을 되읽지는 않는다. 자산을 한 번씩 **다시 검사**
> 하면 채워진다(검사 이력의 `다시 검사` — SBOM 을 다시 올릴 필요 없다).
...
거르개가 그대로 적용된다.** 패키지 내보내기는 화면의 200개 상한을 따르지
