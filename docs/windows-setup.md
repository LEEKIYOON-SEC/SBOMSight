...
초기화된 Windows 11 PC 에 SBOMSight 를 처음부터 설치하는 절차다. 위에서부터
...
**디스크 여유 10GB 이상.** 취약점 DB 가 풀리면 커지고, 업로드한 SBOM 원본과 grype
...
**자산 → 자산 등록.** 서버 한 대가 자산 하나다. 관리 단위는 검사가 아니라
서버다.

여러 대를 한 번에 넣으려면 **자산 → 일괄 등록** 에서 CSV 를 업로드한다.
...
> 나온다. 그대로 업로드해도 된다 — 읽을 때 알아서 가른다. 어떤 인코딩으로 읽었는지
...
자산 상세 화면의 **SBOM 업로드** 에서 파일을 고르고 **검사 시작.** grype 이
뒤에서 돌고, 끝나면 검사 이력에 나타난다. 서버 전체 SBOM 은 수백 MB 가 예사고
grype 도 몇 분 걸린다.

### 취약점 보기

**취약점** — 심각도·수정 버전 유무·실제 악용으로 거르고 정렬한다. 누르면 바로
걸린다(적용 버튼이 없다). 자산 하나만 보려면 그 자산 상세의 `취약점` 탭으로
들어간다 — 한 자산 이야기를 보러 왔다가 전체 목록으로 쫓겨나지 않는다.

행의 **작성** 을 누르면 검토 결과를 남긴다. `해당 없음`·`오탐` 으로 적으면
목록에서 빠지지만 **보고서의 탐지 건수는 그대로다** — grype 이 낸 것은 탐지,
우리가 적는 것은 검토 결과이고 둘을 섞지 않는다.

### 패키지 보기

**패키지** — 무엇이 어디에 몇 버전으로 깔려 있나. 취약점이 하나도 없는
패키지도 여기 있다. `openssl` 을 검색하면 자산마다 어느 버전인지 한 화면에
뜬다 — CVE 가 터진 다음에 찾아 헤매지 않는다.

가장 중요한 신호는 **일부만 업그레이드** 딱지다. 같은 패키지에 취약한 버전과 안전한
버전이 섞여 있다는 뜻이고, 곧 "이미 업그레이드한 자산이 있는데 안 한 자산이 남았다"
는 뜻이다. `일부 자산만 업그레이드` 필터로 그것만 볼 수 있다.

> **업그레이드 직후에는 이 화면이 비어 있다.** 패키지 목록은 검사할 때 SBOM 에서
> 담기고, 이미 쌓인 SBOM 을 되읽지는 않는다. 자산을 한 번씩 **다시 검사**
> 하면 채워진다(검사 이력의 `다시 검사` — SBOM 을 다시 업로드할 필요 없다).

### 보고서

**보고서** 에서 무엇을 뽑을지 먼저 고른다.

- **자산 한 대** — `자산별 · 최신 검사` 목록에서 고른다. 문서 정보 + 번호가
  붙은 6장(점검 개요 · 점검 결과 요약 · 조치 대상 · 수정 버전 없는 항목 ·
  조치 진행 현황 · 이전 검사 대비). 인쇄하면 그대로 PDF 가 된다. 6장 아래
  **최근 검사 추이**는 그 검사까지의 완료 검사 최대 여섯 번(탐지 · 심각 · 높음과
  그때의 grype · 취약점 DB)이다 — 실패한 검사는 넣지 않는다.
- **구역 · 기간** — "9월 DMZ 현황". 구역과 기간을 고르면 그 구역 전체를 한
  장으로 낸다. **기간 안에 검사되지 않은 자산을 먼저 이름까지 밝힌다** — 서른
  대 중 열 대만 검사하고 "탐지 1,200건" 이라고 쓰면 구역의 현황이 아니다.
  돌리지 않은 자산(`검사 기록이 없는 자산`)과 돌렸는데 실패만 한 자산
  (`검사가 모두 실패한 자산`)을 가른다. 자산마다 기간 안의 마지막 **완료** 검사로
  세므로, 그 뒤에 검사가 실패했으면 3장 그 자산 줄에 `이후 검사 실패` 와 시각이 붙는다.
- 둘 다 첫 쪽이 **요약**이다: 뒤 장들이 센 수(탐지 · 심각 · 원격 접근 · 미검토 ·
  조치 대상 · 조치 진행 · 증감)를 그 장 번호와 함께, 그리고 조치 대상 상위 5개.
  새로 세지 않는다 — 요약과 본문의 수가 늘 같다.
- 둘 다 끝에 **부록 — 실제 악용 · 심각 취약점**: 취약점마다 한 줄, 검사 결과에
  든 취약점 설명의 원문(대개 영문) 첫 문장. 전문은 화면의 CVE 상세에 있다.

> 산문이 없다. 표와 각주뿐이고, 각주는 그 표를 어떤 기준으로 세었는지만
> 적는다. 심각도·CVSS·수정 버전·취약점 설명은 grype 원본을 그대로 옮긴다.

### CSV 내려받기

취약점 · 대응 · 감사 로그 · **패키지** 화면 머리에 있다. **화면에 걸어 둔
필터가 그대로 적용된다.** 패키지 내보내기는 화면의 200개 상한을 따르지
않는다 — 거른 것 전부를 낸다. 엑셀에서 바로 열린다(UTF-8 BOM).

취약점 CSV 의 `심각도` · `수정 상태` 는 화면과 같은 말(`심각` · `수정 버전 없음`
…)이다. 검사 결과의 글자는 맨 뒤 `심각도 원문` · `수정 상태 원문` 칸에 그대로
있다 — 화면에서는 둘 다 `수정 버전 없음` 인 `wont-fix` 와 `not-fixed` 를 거기서
가른다.
...
띄운다** — 윈도우에 들어 있는 것이라 폐쇄망에 따로 반입할 것이 없다.
...
```powershell
.\scripts\install-service.ps1 -Stop      # 내린다 (업그레이드 전에 반드시)
...
에서 담기고, 업그레이드가 이미 쌓인 SBOM 을 되읽지는 않는다.
...
6단계로 돌아간다. 실행은 되는데 실패한다면 오류 문구를 본다 — 자산 상세의 개요
위(가장 최근 검사가 실패했을 때)와 검사 이력에 뜬다.
시간 초과라면 기본 60분을 늘린다(`$env:SBOMSIGHT_GRYPE_TIMEOUT = '120'`).

고친 뒤에는 그 자리의 **`다시 검사`** 를 누른다. SBOM 은 올린 순간 보관되므로
다시 업로드할 필요가 없다.
...
그 grype 버전이 주지 않은 것이다. **0 이나 "없음" 으로 채우지 않는다** — 확인하지
못한 것과 없는 것은 다른 이야기다. 보고서에도 그렇게 적힌다. 최신 버전으로
...
| 문서 | 내용 |
|---|---|
| [`docs/operations.md`](operations.md) | **설치한 뒤 — 서비스 다루기 · 운영 리듬 · 백업 · 업그레이드** |
